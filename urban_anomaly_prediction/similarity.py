...
graph = numpy.ndarray(numpy.zeros([len(queries), len(ads)]))
...
query_sim = numpy.identity(len(queries))
ad_sim = numpy.identity(len(ads))
...
def simrank(C=0.8, times=1):
    ...
    for run in range(times):
        # queries simrank
        new_query_sim = numpy.identity(len(queries))
        for qi in queries:
            for qj in queries:
                i = queries.index(qi)
                j = queries.index(qj)
        ...
        # ads simrank
        new_ad_sim = numpy.identity(len(ads))
        for ai in ads:
            for aj in ads:
                i = ads.index(ai)
                j = ads.index(aj)
